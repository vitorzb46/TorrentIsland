...
public class TorrentLoopRenderer(ClientEngine engine, ConsoleLogRenderer renderer, ILogger<TorrentLoopRenderer> logger, TorrentRepository repository) : ITorrentLoopRenderer
{
    ...
    private TorrentRepository Repository { get; } = repository;
    ...
    public async Task TorrentInfoRender(IProgress<double>? progress = null)
    {
        ...
        var managers = Repository.Managers;

        while (Engine.IsRunning)
        {
            if (managers.Count == 0 || managers.Select(m => m.Value.State)
                                               .Any(s => s == TorrentState.Seeding || s == TorrentState.Stopped))
            {
            ...
            foreach (var (id, _) in managers)
            {
                var p = await Repository.ObterAsync(id);
