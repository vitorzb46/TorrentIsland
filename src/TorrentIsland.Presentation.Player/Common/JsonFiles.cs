...
public class JsonFiles
{
    ...
    public static async Task SaveToFileAsync<TKey, TValue>(IDictionary<TKey, TValue> instance, string cachePath)
    {
    ...
    public static async Task LoadFromFileAsync<TKey, TValue>(string cachePath, IDictionary<TKey, TValue> instance)
    {
        await _semaphore.WaitAsync();
        try
        {
        ...
        {
            Log.Salvar($"Erro ao carregar timestamp cache: {ex.Message}");
        }
        finally
        {
            _semaphore.Release();
